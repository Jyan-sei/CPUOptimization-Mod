...
- turns off lights that are too far from the camera (radius follows your view by default)
- on some traps (jumppad, coil, turret): kills the light2d and fakes the glow by flashing the sprite instead
- **spike traps are left alone** - still get the blinking red light
...
**mp host:** union of every living player's 2×2 - so remote players' areas still sim on the server.

**mp client:** local 2×2 only. you don't pay for simming someone else's side of the map.

stuff outside the window isn't gone - it wakes back up when a window covers it. that's the tradeoff.

config section: `[ChunkSim]`

other mods can ask "is this world pos in the sim window?" via `CPUOptimization.ChunkSimQuery` (krokmpoptimization2 uses this for registry cull).
...
defaults are tuned for mp host + deep base. if lights pop in/out weird, tweak `[Lights] CullRadius` / `CullCameraMargin`. if items feel frozen until you walk closer, that's chunk sim working - widen isn't really an option without losing the point.
